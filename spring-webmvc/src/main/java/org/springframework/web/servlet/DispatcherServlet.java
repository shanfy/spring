...
public class DispatcherServlet extends FrameworkServlet {
	 ...
	 * 最终是容器启动时执行refresh中的finishRefresh触发了事件
	 */
	@Override
	...
	private void initHandlerMappings(ApplicationContext context) {
		...
		if (this.detectAllHandlerMappings) {
			...
			// 按照HandlerMapping.class类型找到容器中所有的HandlerMapping
		...
		if (this.handlerMappings == null) {
			...
			// 如果按照类型和按照固定id从ioc容器中找不到对应组件，则会按照默认策略进行注册初始化，默
			// 认策略在 DispatcherServlet.properties文件中配置
	...
	protected <T> List<T> getDefaultStrategies(ApplicationContext context, Class<T> strategyInterface) {
		...
		//从 DispatcherServlet.properties 配置文件中查找默认策略
		//路径，本jar下，resources目录中：org/springframework/web/servlet/DispatcherServlet.properties
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				//默认进入：org.springframework.web.servlet.mvc.method.AbstractHandlerMethodAdapter.handle
	...
	protected void render(ModelAndView mv, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//封装View视图对象之后，调用了view对象的render方法 进行数据渲染
	...
	protected View resolveViewName(String viewName, @Nullable Map<String, Object> model,
		...
		if (this.viewResolvers != null) {
			//默认只有一个视图解析器  InternalResourceView 我们配置的前缀和后缀就是在这里体现并解析的
