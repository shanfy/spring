...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	private String[] doGetBeanNamesForType(ResolvableType type, boolean includeNonSingletons, boolean allowEagerInit) {
		...
		for (String beanName : this.beanDefinitionNames) {
			...
			if (!isAlias(beanName)) {
				try {
					// 获取合并的BeanDefinition，合并的BeanDefinition 指的是整合了父BeanDefinition的属性，然后属性值会转换为RootBeanDefinition
	...
	public void preInstantiateSingletons() throws BeansException {
		...
		// 从之前注册的BeanDefinition name集合中获取所有待实例化的bean名称
		List<String> beanNames = new ArrayList<>(this.beanDefinitionNames);

		// Trigger initialization of all non-lazy singleton beans...
		// 触发所有非延迟加载单例bean的初始化，遍历集合的对象
		for (String beanName : beanNames) {
			// 合并父类BeanDefinition,因为bean本身存在父子关系，子bean可以继承父bean的属性，这里进行合并
			RootBeanDefinition bd = getMergedLocalBeanDefinition(beanName);
			// 条件判断，实例化所有非抽象（Abstract标签用来定义抽象类，不为此类创建对象，常与parent配合使用），单例且设置为非懒加载的bean
			if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) {
				// spring中的bean分为工厂bean和普通bean,但是都会走getBean方法进行处理
				if (isFactoryBean(beanName)) {
					// 如果是工厂bean, 要获取其本身对象，需要加前缀 &
	...
	public Object resolveDependency(DependencyDescriptor descriptor, @Nullable String requestingBeanName,
		...
		if (Optional.class == descriptor.getDependencyType()) {
			// 创建0ptiona亿类型的符合descriptor要求的候选Bean对象
		...
		// javaxInjectProviderCLass有可能导致空指针，不过一般情况下，我们引用Spirng包的时候都有引入该类以防止空指针
		// 如果依赖项是javax.inject.Provider
		else if (javaxInjectProviderClass == descriptor.getDependencyType()) {
			// Jse330Provider: javax.inject.Provider实现类,与Dependency0bjectProvoid作用一样，也是用于延退解析依赖
			...
			// 将于descriptor所包装的的对象匹配的候选bean对象返回
			return result;
		}
	}

	/**
	 * 解所出与descriptor所包装的对象匹配的候选Bean对象
	 */
	@Nullable
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		try {
			...
			// 尝试针对desciptor所包装的对象类型是[stream,数组,Collection类型且对象类型是接口,Map]的情况，进行解析与依赖类
			// 针对desciptor所包装的对象类型是[stream,数组,Collection类型且对象类型是接口,Map]的情况，进行解析与依赖类型匹
			// 并将其封装成相应的依赖类型对象
			Object multipleBeans = resolveMultipleBeans(descriptor, beanName, autowiredBeanNames, typeConverter);
			// 如果multpLeBeans不为nuTl
			...
			if (instanceCandidate instanceof Class) {
				// 让instanceCandidate引用descriptor对autowiredBeanName 解析为该工厂的Bean实例，内部调用getBean方法
				instanceCandidate = descriptor.resolveCandidate(autowiredBeanName, type, this);
			}
			// 定义一个result变量，用于存储最佳候选Bean对象
			Object result = instanceCandidate;
			// 如果result对象是NullBean实例
			if (result instanceof NullBean) {
				// 如果descriptor需要注入
				if (isRequired(descriptor)) {
					// 抛出NoSuchBeanDefinitionException或BeanNot0fRequiredTypeException以解决不可 解决的依赖关系
					raiseNoMatchingBeanFound(type, descriptor.getResolvableType(), descriptor);
				}
