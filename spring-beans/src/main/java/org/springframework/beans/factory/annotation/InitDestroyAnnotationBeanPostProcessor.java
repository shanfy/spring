...
public class InitDestroyAnnotationBeanPostProcessor
	...
	// CommonAnnotationBeanPostProcessor 在初始化时向里面放置了@PostDestory注解
	@Nullable
	...
	/**
	 * 构造生命周期元数据(解析带@PostConstruct和@PreDestroy注解的方法)，当其构造完成后会将元数据缓存到LifecycleMetadata
	...
	private LifecycleMetadata buildLifecycleMetadata(final Class<?> clazz) {
		...
		do {
			...
			// 其实就是解析当前类中被@PostConstruct类似生命周期注解标识的方法
	...
	private class LifecycleMetadata {
		...
		/**
		 * 调用前面注册初始化方法集合checkedInitMethods的每一个方法
		 * @param target
		 * @param beanName
		 * @throws Throwable
		 */
